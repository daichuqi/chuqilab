 ...
 */
const OSCILLATION_SPEED_LOW_NOTES = 1.2
const OSCILLATION_SPEED_HIGH_NOTES = 3.0
 ...
 * as a multiplier.
 */
const AMPLITUDE_DAMPEN_LOW_NOTES = 0.92
const AMPLITUDE_DAMPEN_HIGH_NOTES = 0.87
...
export default class Thread {
  constructor(ypPm, pitchPm, strPm, hexPm, indPm, cvPm, suite) {
    this.suite = suite
  ...
  redraw = () => {
    ...
    const cp1x = this.xd - this.dxBez0 + this.xo
    const cp1y = this.yd - this.dyBez0 + this.yo
    const cp2x = this.xd + this.dxBez1 + this.xo
    const cp2y = this.yd + this.dyBez1 + this.yo
    const x = this.xp1 + this.xo
    const y = this.yp1 + this.yo

    this.cv.bezierCurveTo(cp1x, cp1y, cp2x, cp2y, x, y)
    // close path
    this.cv.stroke()
    this.cv.closePath()
  }
