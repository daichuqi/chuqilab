...
class Img extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      prevProps.config.innerWidth !== this.props.config.innerWidth ||
  ...
  processImage = () => {
    ...
    let size =
      this.props.size || this.props.s || config.size || parentContainerWidth
    ...
    const cloudimageUrl = isAdaptive
      ? generateUrl(
          'width',
          getSizeAccordingToPixelRatio(parentContainerWidth),
          filters,
          imgSrc,
          config
        )
    ...
    if (isPreview) {
      ...
      previewCloudimageUrl = isAdaptive
        ? generateUrl(
            'width',
            Math.floor(parentContainerWidth / previewQualityFactor),
            filters,
            imgSrc,
            previewConfig
          )
  ...
  onImageLoad = isPreviewLoaded => {
    ...
    } else if (isPreviewLoaded) this.setState({ isLoaded: true })
    else this.setState({ isPreviewLoaded: true })
  }
  ...
  render() {
    ...
    const imageHeight = Math.floor(
      parentContainerWidth / (ratioBySize || ratio || 1)
    )
    ...
    const imgLoadingStyles = config.imgLoadingAnimation
      ? {
          ...styles.imgWithEffect,
          filter: `blur(${Math.floor(parentContainerWidth / 100)}px)`,
        }
      : {}
    const imgLoadedStyles =
      isLoaded && config.imgLoadingAnimation ? styles.imgLoaded : {}

    const picture = (
      <picture
        className={`${className} cloudimage-image-picture cloudimage-image-${
      ...
      >
        ...
        <img
          ...
          alt={alt}
          onLoad={() => {
            this.onImageLoad(isPreviewLoaded)
          }}
          {...otherProps}
        />
      </picture>
